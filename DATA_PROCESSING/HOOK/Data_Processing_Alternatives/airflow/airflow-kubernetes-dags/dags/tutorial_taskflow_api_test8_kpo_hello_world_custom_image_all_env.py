...
# -----------------------------
@dag(
    dag_id="tutorial_taskflow_api_test8_kpo_hello_world_custom_image_all_env",
    schedule="@once",
    catchup=False,
    ...
    Notice: 
    Here we build on test7 showing a simpler way of injecting secrets globally.
    That we inject all secrets as environment variables using env_from, which is an alternative to passing them individually in the secrets parameter. 
    This way we can inject a whole set of secrets from a Kubernetes Secret into the pod's environment.
    """,
    default_args=default_args,
)
def tutorial_taskflow_api_test8_kpo_hello_world_custom_image_all_env():
    ...
    @task.kubernetes(
        ...
        # Here we inject all secrets as environment variables using env_from, which is an alternative to passing them individually in the secrets parameter.
        # This way we can inject a whole set of secrets from a Kubernetes Secret into the pod's environment.
        env_from=[
            k8s.V1EnvFromSource(secret_ref=k8s.V1SecretEnvSource(name="my-env-secret"))
        ],
        do_xcom_push=True,
    )
...
# Instantiate the DAG
# -----------------------------
dag = tutorial_taskflow_api_test8_kpo_hello_world_custom_image_all_env()
